...
export class DataUploadController {
    ...
    async upload1cInvoices(
        ...
        try {
            // 4. Получаем экземпляр Use Case из DI контейнера
            const process1cInvoiceUploadUseCase = container.resolve(
                Process1cInvoiceUploadUseCase,
            );

            // 5. Запускаем обработку файла, передавая путь и ID пользователя
            const result = await process1cInvoiceUploadUseCase.execute(
                filePath,
                userId,
            );
            ...
            console.error(
                '[DataUploadController] Error during 1C invoice upload execution:',
                error,
            );
            next(error);
        }
