...
const fs_1 = __importDefault(require("fs")); // Импортируем модуль Node.js для работы с файловой системой
const path_1 = __importDefault(require("path")); // Импортируем модуль Node.js для работы с путями
...
function getPackageVersion() {
    try {
        ...
        const packageJsonPath = path_1.default.resolve(__dirname, '../package.json'); // Поднимаемся на 2 уровня до корня проекта
        // Проверяем существование файла перед чтением
        if (!fs_1.default.existsSync(packageJsonPath)) {
            console.error(`package.json not found at expected path: ${packageJsonPath}`);
            return '1.0.0';
        }
...
const options = {
    ...
    definition: {
        ...
        servers: [
            {
                url: `http://localhost:${config_1.default.port}${config_1.default.apiPrefix}`, // Формируем URL из конфига
                ...
                ValidationErrorResponse: {
                    ...
                    properties: {
                        ...
                        message: {
                            ...
                            example: 'Ошибка валидации: Некорректный формат Email; Отсутствует пароль',
                        },
                    },
